...
const MobileNav = ({user}:MobileNavProps) => {
  ...
  <SheetContent side="left" className='border-none bg-white'>
    <Link href="/" className='gap-1 px-4 flex cursor-pointer items-center gap-2'>
                <Image src="/icons/logo.svg" width={34} height={34} alt='Site logo'  />
                <h1 className='text-26 text-black-1 font-ibm-plex-serif font-bold'>Horizon</h1>
            </Link>
            <div className='mobilenav-sheet'>
                ...
                return(
                    ...
                    <Link href={item.route} key={item.label} className={cn('mobilenav-sheet_close w-full' , {'bg-bank-gradient':isActive})}>
                            ...
                            className={cn({
                                'brightness-[3] invert-o':isActive
                            })} 
                            />
